...
function get_features ($tokens)
{
	...
	foreach ($tokens as $token)
	{
		...
		# 20 type 
		if (preg_match('/(holo|iso|lecto|para)(type)?/i', $word))
		{
			$features[20] = 'possibleType';
		}
		else
		{
			$features[20] = 'notType';
		...
		# 21 = possible latlon
		# alternative degree symbols:
		# ° DEGREE SIGN
		# ̊ COMBINING RING ABOVE
		if (preg_match('/[°̊’”\'"\′″]/u', $word))
		{
			$features[21] = 'possibleLatlan';
		}
		else
		{
			$features[21] = 'notLatlan';
		}
		
		// relative location
		$location = floor(($current_token_number - 1) / count($tokens) * 12);
		$features[22] = $location; // 22 = relative location
